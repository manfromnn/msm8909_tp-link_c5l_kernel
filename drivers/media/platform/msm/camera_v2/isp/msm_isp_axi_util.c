	...
	for (i = 0; i < update_cmd->num_streams; i++) {
		...
		case UPDATE_STREAM_FRAMEDROP_PATTERN: {
			...
			if (stream_info->stream_type == BURST_STREAM) {
				...
				if ((framedrop_pattern == stream_info->framedrop_pattern)
					&& ((framedrop_period - 1) == stream_info->framedrop_period)
					&& (framedrop_period <= 16)
					&& (framedrop_period > 1)
					&& framedrop_pattern) {
					framedrop_pattern = framedrop_pattern + (framedrop_pattern << framedrop_period);
					framedrop_period = framedrop_period * 2;
				}
				stream_info->framedrop_pattern = framedrop_pattern;
				stream_info->framedrop_period = framedrop_period - 1;
				vfe_dev->hw_info->vfe_ops.axi_ops.
