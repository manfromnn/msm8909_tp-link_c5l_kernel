...
#include <linux/fb.h>
//add by shihuijun for tp charge interface updatedate 20150728  
	...
	for (i = 0; i < event->touch_point; i++) {
		...
		if (event->pressure == FT_PRESS) {
			...
			input_report_abs(data->input_dev, ABS_MT_POSITION_Y,
					event->au16_y[i]);
		//comment it by pangle for touch range at 20150924
...
//modify by shihuijun for charger interference to tp 20150728	begin	
	   ...
	   if(!gesture_flag)
    #endif
    //printk(KERN_ERR"%s --- shjwrite 8b register suscess!\n",__func__);
       {
	 ...
	 #endif
//modify by shihuijun for charger interference to tp 20150728	end
		...
		ft5x36_data->pm_status=*blank;
		  ...
		  if (*blank == FB_BLANK_UNBLANK)
		  {
			queue_delayed_work(ft5x36_data->fts_pm_wq,
				&ft5x36_data->fts_pm_work,msecs_to_jiffies(120));     //resume  use 120ms timeout
	        // schedule_delayed_work(&ft5x36_data->fts_pm_work, msecs_to_jiffies(120));	// restart timer
		  }
		  else if(*blank == FB_BLANK_POWERDOWN)
		  {
		      queue_delayed_work(ft5x36_data->fts_pm_wq,
					&ft5x36_data->fts_pm_work,msecs_to_jiffies(0));   //suspend don not use timeout
		      // schedule_delayed_work(&ft5x36_data->fts_pm_work, msecs_to_jiffies(120));	// restart timer
		  }
...
static void ft5x36_hardware_info_reg(u8 family_id,u8 vendor_id,u8 fw_ver)
{
    int i=0;
    ...
    for(i=0;i<sizeof(ft5x36_name)/sizeof(ft5x36_ven_info);i++){
        if(vendor_id==ft5x36_name[i].vendor_id){
...
//modify interrupt thread by shj 20150906
