...
//added by pangle for 860 &861 fw upgrade at 20150601 begin	
	case FT6X36_VENDOR0x9b_JINLONG:
#if defined(CONFIG_QL860_BASE)||defined(CONFIG_QL862_BASE)
...
//added by pangle for 860 & 861 fw upgrade at 20150601 end	

//added by shj for QL613 fw upgrade at 20150824 begin
case FT6X36_FIRMWARE0x67_DIJING:
          printk(KERN_INFO "FT5X36:upgrade with DIJING firmware\n");
          fw_len = sizeof(FT6X36_FIRMWARE0x67_DJ);
          if (fw_len < 8 || fw_len > 32 * 1024) {
                dev_err(&client->dev, "%s:FW length error\n", __func__);
                return -EIO;
              }
              /*FW upgrade */
              pbt_buf = FT6X36_FIRMWARE0x67_DJ;
              /*call the upgrade function */
              i_ret = fts_ctpm_fw_upgrade(client, pbt_buf, sizeof(FT6X36_FIRMWARE0x67_DJ));
              if (i_ret != 0)
                  dev_err(&client->dev, "%s:upgrade failed. err.\n",
                          __func__);
              else{
    ...
    break;
//added by shj for QL613 fw upgrade at 20150824 end
...
u8 fts_ctpm_get_i_file_ver(struct i2c_client *client)
{
  ...
  //modify by shj for vendor ID error  of QL613 begin 20150824
  case FT6X36_FIRMWARE0x67_DIJING:
	
	ui_sz = sizeof(FT6X36_FIRMWARE0x67_DJ);
	ui_sz = 0x10a;
	if (ui_sz > 2)
		return FT6X36_FIRMWARE0x67_DJ[ui_sz];

  //modify by shj for vendor ID error  of QL610  end 20150824
  break;
