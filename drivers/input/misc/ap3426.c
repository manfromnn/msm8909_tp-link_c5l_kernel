...
//modify by yanfei for sensor calibration 20150702
    if((pxvalue < 400)&&(pxvalue > 0))
    {
    	ps_data->ps_thres_high=value+150;
	ps_data->ps_thres_low =value+60;
	flag =1;
	ps_data->prevalue=value;
    }
    else if(pxvalue <= 0)
    {
    	ps_data->ps_thres_high=ps_data->crosstalk+150;
	ps_data->ps_thres_low =ps_data->crosstalk+60;
	flag =1;
    ...
    else
    {
	    if(flag == 1)
		{
			ps_data->ps_thres_high=ps_data->prevalue+150;
			ps_data->ps_thres_low =ps_data->prevalue+60;
		}
		else
		{
			ps_data->ps_thres_high=ps_data->crosstalk+150;
    ...
    ap3426_set_phthres(ps_data->client,ps_data->ps_thres_high);
	 ...
	 printk(KERN_ERR" %s misc_ls_opened=%d\n",__func__,ps_data->rels_enable);
	    	}
	else
	{
	 ret = __ap3426_write_reg(ps_data->client,
...
//modify for ps-sensor threshold by yanfei 20150923 begin
	data->ps_high=150;
	data->ps_low =60;
	mutex_lock(&data->lock);
...
static int ap3426_init_client(struct i2c_client *client)
{
    ...
    int i;
    int ret;
//add by yanfei for ap3426 reg init config 20150815 begin
    ...
    i2c_smbus_write_byte_data(client, AP3426_REG_PS_INTEGR, 0x06);
    i2c_smbus_write_byte_data(client, AP3426_REG_PS_PERSIS, 0x02);
//modify for ps-sensor config by yanfei 20150815 end
...
static void lsensor_work_handler(struct work_struct *w)
{
	...
	ktime_t timestamp;
	timestamp = ktime_get_boottime();
//modify for als value 2015815 begin	   
...
//modify for als value 2015815 end	   
