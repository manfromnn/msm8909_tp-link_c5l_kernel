...
/*xiongzuan add for battery status check 20150518 end*/		
	struct delayed_work		allow_get_bat_voltage_work;/* Add by lichuangchuang for can not resume phone when remove usb or ac (8909) SW00000000 20150724*/
...
/*Add by xiongzuan for get voltage of battery wakelock (base) SW00000000 begin 20150729*/
static bool allow_get_bat_voltage = true;
static bool allow_get_bat_voltage_work_init = false;
static int64_t last_bat_voltage = 0;
#define GET_BAT_VOLTAGE_NUM 22

static void qpnp_get_bat_voltage_work(struct work_struct *work) 
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct qpnp_lbc_chip *chip = container_of(dwork,
				struct qpnp_lbc_chip, allow_get_bat_voltage_work);
	int rc = 0;
	int max = 3800, mix = 3800;
	long sum = 0;
	int count = 0;
	struct qpnp_vadc_result results;
	pr_debug("qpnp_get_bat_voltage_work   in\n");
	for(count = 0; count < GET_BAT_VOLTAGE_NUM; count++)
	{
		msleep(5);
	...
	last_bat_voltage = results.physical;
	allow_get_bat_voltage = true;
	pr_debug("qpnp_get_bat_voltage_work   out\n");
}
...
static int get_prop_battery_voltage_now(struct qpnp_lbc_chip *chip)
{
	...
	int rc = 0;
	struct qpnp_vadc_result results;
	pr_debug("get_prop_battery_voltage_now   in\n");
	
	if(allow_get_bat_voltage_work_init == false){
		allow_get_bat_voltage_work_init = true;
		INIT_DELAYED_WORK(&chip->allow_get_bat_voltage_work, 
			qpnp_get_bat_voltage_work);
		...
		last_bat_voltage  = sum / count;
		pr_info("allow_get_bat_voltage_work_init  voltage = %lld.\n", last_bat_voltage);
	}
	
	if(allow_get_bat_voltage){
		allow_get_bat_voltage = false;
		schedule_delayed_work(&chip->allow_get_bat_voltage_work, 
			msecs_to_jiffies(0));
	}
	
	pr_debug("get_prop_battery_voltage_now   out voltage = %lld.\n", last_bat_voltage);
	return last_bat_voltage;
}
/*add by xiongzuan for get voltage of battery wakelock (base) SW00000000 end 20150729*/
...
/*xiongzuan modify for battery-health 20151110 begin*/
#define BATT_TEMP_OVERHEAT   (53*10)
#define BATT_TEMP_OVERCOLD   (-3*10)
#define BATT_TEMP_COOL_DEGC        (10*10)
#define BTTE_TEMP_WARM_DEGC     (45*10)
/*xiongzuan modify for battery-health 20151110 end*/
...
#endif
/*xiongzuan modify for Tip temperature is too low when charger inserted 20150714 begin*/
...
/*xiongzuan modify for Tip temperature is too low when charger inserted 20150714 end*/
