...
enum {
	...
	POWER_SUPPLY_BATT_ID_TECNO,
	POWER_SUPPLY_BATT_ID_back,
	};
//aidongdong add for battery id info by 20140804 end
...
//aidongdong add for battery id info by 20140804 begin 
#define  BATT_ID_10K    13000              //BYD
#define  BATT_ID_470K    1485000              //FUJIAN
#define  BATT_ID_BIAS  10000

static int get_prop_bms_info(struct qpnp_bms_chip *chip)
{
	int64_t battery_id;
	int battery_present = is_battery_present(chip);
	int battery_info = 0;
	...
	//pr_info("read battery id = %lld, battery_present = %d.\n",battery_id, battery_present);
	if (battery_present) {
		battery_info = POWER_SUPPLY_BATT_ID_TECNO;
	//}else if ((battery_id > (BATT_ID_470K - BATT_ID_BIAS)) && (battery_id < (BATT_ID_470K + BATT_ID_BIAS))) {
	//	battery_info = POWER_SUPPLY_BATT_ID_back;
	}else {
...
static int set_battery_data(struct qpnp_bms_chip *chip)
{
	...
	if((chip->battery_id_info)==POWER_SUPPLY_BATT_ID_TECNO){
		node = of_find_node_by_name(chip->spmi->dev.of_node,
					"qcom,battery-data");
                   pr_err("load tecno battery\n");
	}else{
		node = of_find_node_by_name(chip->spmi->dev.of_node,
					"qcom,battery-data");
                    pr_err("load no battery\n");
	}	
	if (!node) {
